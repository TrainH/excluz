...
class CartItemV3ServiceTest {
	...
	// 실제 객체
	@InjectMocks
	private CartItemV3Service cartItemV3Service;
	...
	void getCartItemList() {
		...
		// when
		CartItemListResponseDto result = cartItemV3Service.getCartItemList(user.getId(), userRole, page, size);
	...
	void getCartItemListEmptyCart() {
		...
		// when
		CartItemListResponseDto result = cartItemV3Service.getCartItemList(userId, userRole, page, size);
	...
	void getCartItemListInvalidUserRole() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.getCartItemList(userId, userRole, page, size))
	...
	void addItemToCart() {
		...
		// when
		CreateCartItemResponseDto result = cartItemV3Service.addItemToCart(userId, userRole, requestDto);
	...
	void addItemToCartUserNotFound() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.addItemToCart(userId, userRole, requestDto))
	...
	void getCartItem() {
		...
		// when
		GetCartItemResponseDto result = cartItemV3Service.getCartItem(user.getId(), userRole, cartItem.getId());
	...
	void getCartItemNotFound() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.getCartItem(user.getId(), userRole, cartItemId))
			.isInstanceOf(NotFoundException.class); // NotFoundException 예외 발생
	...
	void getCartItemInvalidUserRole() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.getCartItem(userId, userRole, cartItemId))
			.isInstanceOf(ForbiddenException.class); // ForbiddenException 예외 발생
	}
	...
	void getCartItemWrongUser() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.getCartItem(userA.getId(), userRole, cartItem.getId()))
			.isInstanceOf(NotFoundException.class); // NotFoundException 예외 발생
	...
	void updateCartItemQuantity() {
		...
		// when, then
		Assertions.assertThatCode(() -> cartItemV3Service.updateCartItemQuantity(
				user.getId(),
				userRole,
				cartItem.getId(),
	...
	void updateCartItemQuantityBadRequest() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.updateCartItemQuantity(
				user.getId(),
				userRole,
				cartItem.getId(),
				requestDto // 업데이트할 장바구니 아이템 정보
	...
	void removeCartItemCartItemNotFound() {
		...
		// when, then
		Assertions.assertThatThrownBy(() -> cartItemV3Service.removeCartItem(userId, userRole, cartItemId))
			.isInstanceOf(NotFoundException.class); // NotFoundException 예외 발생
